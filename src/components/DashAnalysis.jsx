...
const DashAnalysis = () => {
  ...
  const getReviews = async () => {
    try {
      ...
      const docs = snapshots.docs.map((doc) => ({...doc.data(), createdAt: moment(doc.data().createdAt.toDate().toISOString()).format('DD MMM YYYY') }))   
...
const data = {
  ...
  datasets: [{
    ...
    data: formattedDocsData ? Object.values(formattedDocsData).map((arr) => arr.length) : [],
...
<h1 className='analysis-title'>Line Chart Showing the Number of Reviews per Day</h1>
